...
NS_LOG_COMPONENT_DEFINE("AodvAtkRoutingProtocol");
...
uint64_t GetBhSeen() { return g_bhFwdSeen; }
NS_OBJECT_ENSURE_REGISTERED(RoutingProtocol);

/// UDP Port for AODV control traffic
const uint32_t RoutingProtocol::AODVATK_PORT = 654;
...
class DeferredRouteOutputTag : public Tag
{
  public:
    ...
    {
        static TypeId tid = TypeId("ns3::aodvatkatk::DeferredRouteOutputTag")
                                .SetParent<Tag>()
                                .SetGroupName("Aodv")
...
{
    static TypeId tid =
        TypeId("ns3::aodvatkatk::RoutingProtocol")
            ...
            .AddAttribute("DropProb",
                          "Probability of discarding a forwarded data packet",
                          DoubleValue(1.0),
                          MakeDoubleAccessor(&RoutingProtocol::m_dropProb),
        ...
        if (m_ipv4->GetInterfaceForAddress(iface.GetLocal()) == iif)
        {
            if (dst == iface.GetBroadcast() || dst.IsBroadcast())
            {
                ...
                if (header.GetProtocol() == UdpL4Protocol::PROT_NUMBER)
                {
                    ...
                    if (udpHeader.GetDestinationPort() == AODVATK_PORT)
                    {
...
RoutingProtocol::Forwarding(Ptr<const Packet> p,
                            ...
                            ErrorCallback ecb)
{
    // ---- Black hole / grey hole data-plane drop --------------------
    // Reached only for packets this node agreed to relay. Discarding here
    ...
    if (m_blackHole)
    {
        ...
        if (!m_dropRv)
        {
            m_dropRv = CreateObject<UniformRandomVariable>();
        }
...
RoutingProtocol::NotifyInterfaceUp(uint32_t i)
{
    ...
    socket->Bind(InetSocketAddress(iface.GetLocal(), AODVATK_PORT));
    ...
    socket->SetRecvCallback(MakeCallback(&RoutingProtocol::RecvAodv, this));
    socket->BindToNetDevice(l3->GetNetDevice(i));
    socket->Bind(InetSocketAddress(iface.GetBroadcast(), AODVATK_PORT));
    socket->SetAllowBroadcast(true);
    socket->SetIpRecvTtl(true);
...
{
    socket->SendTo(packet, 0, InetSocketAddress(destination, AODVATK_PORT));
}
...
RoutingProtocol::RecvRequest(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
    ...
    if (m_blackHole && m_forgeRrep)
    {
        ...
        {
            Ptr<NetDevice> dev = m_ipv4->GetNetDevice(m_ipv4->GetInterfaceForAddress(receiver));
            RoutingTableEntry ne(dev, origin, true, rreqHeader.GetOriginSeqno(),
                m_ipv4->GetAddress(m_ipv4->GetInterfaceForAddress(receiver), 0),
                1, src, m_activeRouteTimeout);
            m_routingTable.AddRoute(ne);
            m_routingTable.LookupRoute(origin, toOrigin);
        }
        RreqHeader forged = rreqHeader;
        forged.SetDstSeqno(rreqHeader.GetDstSeqno() + 100);
        forged.SetHopCount(0);
        SendReply(forged, toOrigin);
        return;
    }
...
RoutingProtocol::SendReply(const RreqHeader& rreqHeader, const RoutingTableEntry& toOrigin)
{
    ...
    NS_ASSERT(socket);
    socket->SendTo(packet, 0, InetSocketAddress(toOrigin.GetNextHop(), AODVATK_PORT));
}
                                             ...
                                             bool gratRep)
{
    ...
    if (gratRep)
    {
        ...
        socket->SendTo(packetToDst, 0, InetSocketAddress(toDst.GetNextHop(), AODVATK_PORT));
    }
}
...
RoutingProtocol::SendReplyAck(Ipv4Address neighbor)
{
    ...
    NS_ASSERT(socket);
    socket->SendTo(packet, 0, InetSocketAddress(neighbor, AODVATK_PORT));
}
                                              ...
                                              Ipv4Address origin)
{
    ...
    else
    {
        for (auto i = m_socketAddresses.begin(); i != m_socketAddresses.end(); ++i)
        {
            ...
            socket->SendTo(packet->Copy(), 0, InetSocketAddress(destination, AODVATK_PORT));
        }
    }
}
